...
void Branch_Wifi::init(ScreenRouter* router, Helper_Preferences* prefs, DeviceConfig* device,
    ...
    httpEndpoints->setPreferences(preferences);  // Inject preferences helper
...
void Branch_Wifi::handlePreferencesRequest(const JsonDocument& doc) {
    ...
    if (doc.containsKey("action")) {
        ...
        if (action == "get") {
            ...
            // Server sends "wifiSSID" not "ssid"
            if (data.containsKey("wifiSSID")) {
                String newSSID = data["wifiSSID"].as<String>();
                if (newSSID != ssid) {
            ...
            // Server sends "wifiPassword" not "password"
            // Only update if non-empty (server returns "" for security, we don't want to overwrite)
            if (data.containsKey("wifiPassword")) {
                String newPassword = data["wifiPassword"].as<String>();
                if (newPassword.length() > 0) {
                    ...
                    Serial.println("[Branch_Wifi] Skipping empty password (keeping existing)");
                }
            }

            // Note: Server does not send deviceName or autoReconnect in current implementation
            ...
            if (data.containsKey("connMode")) {
                ...
                Serial.printf("[Branch_Wifi] Connection mode updated to: %s\n", connMode.c_str());
            }
            ...
            if (data.containsKey("rotation")) {
                ...
                Serial.printf("[Branch_Wifi] Display rotation updated to: %d\n", rotation);
            }
            ...
            if (data.containsKey("swapBlueGreen")) {
                ...
                Serial.printf("[Branch_Wifi] Swap B/G channels set to: %s\n", swapBG ? "true" : "false");
            }
            ...
            if (data.containsKey("mqttPassword")) {
                ...
                if (mqttPassword.length() > 0) {
                    ...
                    Serial.println("[Branch_Wifi] MQTT password updated");
                } else {
                    Serial.println("[Branch_Wifi] Skipping empty MQTT password (keeping existing)");
                }
            }
            ...
            if (data.containsKey("externalNeoPixelsData1")) {
                ...
                Serial.printf("[Branch_Wifi] External NeoPixels Data1 updated to: %s\n", neoPixel1.c_str());
            }
            ...
            if (data.containsKey("externalNeoPixelsData2")) {
                ...
                Serial.printf("[Branch_Wifi] External NeoPixels Data2 updated to: %s\n", neoPixel2.c_str());
            }
            ...
            if (data.containsKey("restart") && data["restart"].as<bool>()) {
                Serial.println("[Branch_Wifi] Restart requested, restarting device in 2 seconds...");
                delay(2000);
                ESP.restart();
            }
        }
