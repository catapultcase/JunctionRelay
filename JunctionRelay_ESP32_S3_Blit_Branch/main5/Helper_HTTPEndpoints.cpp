...
void Helper_HTTPEndpoints::handleGetPreferences(AsyncWebServerRequest* req) {
    Serial.println("[Helper_HTTPEndpoints] Get preferences request received");

    if (!preferences) {
        Serial.println("[Helper_HTTPEndpoints] ERROR: Preferences helper not available");
        req->send(500, "application/json", "{\"error\":\"Preferences not available\"}");
        return;
    }

    // Get all preferences as JSON
    String preferencesJSON = preferences->getAllSettingsJSON();

    if (preferencesJSON.isEmpty() || preferencesJSON == "{}") {
        Serial.println("[Helper_HTTPEndpoints] WARNING: Preferences JSON is empty");
        req->send(200, "application/json", "{}");
        return;
    }

    Serial.printf("[Helper_HTTPEndpoints] Sending preferences (%d bytes)\n", preferencesJSON.length());
    req->send(200, "application/json", preferencesJSON);
}
