...
#define FIRMWARE_VERSION "JunctionRelay v0.8.9_Blit"
...
inline String getFormattedMacAddress() {
    static String macCache;             // holds the one-time result
    if (macCache.isEmpty()) {           // only on first call…
        ...
        macCache = String(macStr);      // cache it
    }
    return macCache;                    // subsequent calls just return cached
}
