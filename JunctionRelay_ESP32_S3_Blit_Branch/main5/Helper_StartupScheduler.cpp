...
void Helper_StartupScheduler::createManagers(const HardwareInventory& inventory) {
    ...
    // Always create Blit manager in blit branch (supports onboard, matrix, and I2C displays)
...
void Helper_StartupScheduler::setupBlitManager() {
    ...
    // Blit branch always creates blit manager - it will auto-detect display type
    // (onboard screen, external matrix, or I2C CharliePlex)
...
void Helper_StartupScheduler::startManagerTasks() {
    ...
    // Start Matrix manager early (blit manager may need it)
    if (matrixManager) {
    ...
    // Start I2C managers early (blit manager may need them)
    if (quadDisplayManager) {
    ...
    // Start Blit manager AFTER external displays are initialized
    if (blitManager) {
