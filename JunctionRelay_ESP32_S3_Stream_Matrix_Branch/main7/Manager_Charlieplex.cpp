...
Manager_Charlieplex::Manager_Charlieplex(TwoWire* wireInterface)
    : wireInterface(wireInterface), charliplexTaskHandle(nullptr), taskRunning(false), blitModeActive(false), taskStarted(false) {
...
void Manager_Charlieplex::begin() {
    ...
    for (auto& pair : displays) {
        ...
        info.display.setRotation(2);
        
        info.initialized = true;
        info.currentBrightness = 90; // Track the initial brightness
        
        Serial.printf("[CHARLIEPLEX] Successfully initialized display at 0x%02X with TomThumb font and 180° rotation\n", address);
    }
...
void Manager_Charlieplex::internalUpdate() {
    // Skip updates when blit mode is active (blit renderer is controlling the display)
    if (blitModeActive) {
        return;
    }
    updateScrollingText();
...
Adafruit_IS31FL3731* Manager_Charlieplex::getDisplay(uint8_t address) {
...
void Manager_Charlieplex::setBlitMode(bool active) {
    blitModeActive = active;
    if (active) {
        Serial.println("[CHARLIEPLEX] Blit mode enabled - pausing scrolling text");
    } else {
        Serial.println("[CHARLIEPLEX] Blit mode disabled - resuming scrolling text");
    }
}
...
void Manager_Charlieplex::setBrightness(uint8_t brightness, uint8_t address) {
    executeOnDisplay(address, [this, brightness](uint8_t addr) {
        ...
        if (info.currentBrightness != brightness) {
            info.brightness = brightness;
            info.currentBrightness = brightness;
            // For IS31FL3731, brightness is controlled via text color
            // Store the brightness value and it will be applied when text is displayed
            Serial.printf("[CHARLIEPLEX] Set brightness to %d for display 0x%02X\n", brightness, addr);
        }
    });
}
...
void Manager_Charlieplex::displayTextOnScreen(uint8_t address, const char* text) {
    ...
    // Set text color based on current brightness (0-255)
    uint16_t textColor = info.brightness;
    ...
    // For TomThumb font with 180° rotation, adjust cursor position
    matrix.setCursor(0, 6);         // 3 - Set position (adjusted for TomThumb font baseline and rotation)
    matrix.setFont(&TomThumb);      // 4 - Ensure TomThumb font is set
    matrix.setTextColor(textColor); // 5 - Set brightness through text color
    matrix.print(text);             // 6+ - Write text (varies by length)
    matrix.displayFrame(nextFrame); // 7+ - Display the frame
    ...
    for (JsonPairConst kv : sensors) {
        ...
        if (strcmp(sensorName, "jr_brightness") == 0) {
            if (dataItem.containsKey("Value")) {
                // Get brightness value (0-255) and use directly for IS31FL3731
                ...
                uint8_t displayBrightness = (uint8_t)brightnessValue;
                
                // Update brightness for target display(s)
                setBrightness(displayBrightness, itemTargetAddress);
            }
        ...
        // 6) Display to the specific display (or broadcast if targetAddress == 0)
        if (itemTargetAddress == 0) {
            // broadcast
            for (auto& pair : displays) {
                if (pair.second.initialized) {
            ...
            // single display
            auto it = displays.find(itemTargetAddress);
            if (it != displays.end() && it->second.initialized) {
                displayTextOnScreen(itemTargetAddress, displayText.c_str());
            } else {
