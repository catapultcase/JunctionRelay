...
Manager_I2C::Manager_I2C(Manager_Connections* connMgr, TwoWire* wireInterface, int sdaPin, int sclPin) 
    : connMgr(connMgr), mqttManager(nullptr), wire(wireInterface), encoder_position(0), initialized(false), taskHandle(NULL), ss(wireInterface), scanResultsAvailable(false) {
    
    // Store I2C pin configuration
    this->sdaPin = sdaPin;
    this->sclPin = sclPin;
    
    // Initialize encoder tracking arrays
    for (int i = 0; i < MAX_ENCODERS; i++) {
        encoderPositions[i] = 0;
        encoderAddresses[i] = 0;
        seesawInstances[i] = nullptr;
        buttonStates[i] = false; // Initialize all buttons as not pressed
    }
    encoderCount = 0;
...
int Manager_I2C::scanWithOriginalMethod(JsonArray& screens, JsonArray& i2cDevices, const String& devicePrefix) {
    ...
    for(address = 1; address < 127; address++) {
        ...
        if (error == 0) {
            ...
            if (isRotaryEncoderAddress(address)) {
                foundSeesaw = true;
                configureRotaryEncoderDevice(i2cDevices, address, devicePrefix);
            }
...
bool Manager_I2C::isRotaryEncoderAddress(uint8_t address) {
    // Check if address is in the rotary encoder range (0x36-0x3D)
    return (address >= 0x36 && address <= 0x3D);
}

int Manager_I2C::getEncoderNumber(uint8_t address) {
    // Convert address to encoder number (0x36 = encoder 1, 0x37 = encoder 2, etc.)
    if (isRotaryEncoderAddress(address)) {
        return (address - 0x36) + 1;
    }
...
void Manager_I2C::initializeManagers(bool foundSeesaw, bool foundQuadDisplay, bool foundCharlieplex, const String& devicePrefix) {
    ...
    if (foundSeesaw) {
        Serial.printf("[I2C] Found %d Seesaw encoder(s) - will initialize when begin() is called\n", encoderCount);
    }
...
void Manager_I2C::configureRotaryEncoderDevice(JsonArray& i2cDevices, uint8_t address, const String& devicePrefix) {
    int encoderNum = getEncoderNumber(address);
    Serial.printf("[I2C] Configuring Rotary Encoder #%d at address 0x%02X\n", encoderNum, address);
    
    // Store encoder address for later initialization
    if (encoderCount < MAX_ENCODERS) {
        encoderAddresses[encoderCount] = address;
        encoderCount++;
    }
    
    JsonObject i2cDevice = i2cDevices.createNestedObject();
    i2cDevice["I2CAddress"] = "0x" + String(address, HEX);
    i2cDevice["DeviceType"] = "Rotary Encoder with Button #" + String(encoderNum);
    i2cDevice["EncoderNumber"] = encoderNum;
    ...
    endpoint1["Address"] = "JunctionRelay/" + devicePrefix + "/encoder" + String(encoderNum) + "/button";
    endpoint1["QoS"] = 1;
    endpoint1["Notes"] = "Button press detection for encoder " + String(encoderNum);

    JsonObject endpoint2 = endpoints.createNestedObject();
    endpoint2["EndpointType"] = "Encoder";
    endpoint2["Address"] = "JunctionRelay/" + devicePrefix + "/encoder" + String(encoderNum) + "/position";
    endpoint2["QoS"] = 1;
    endpoint2["Notes"] = "Encoder position for encoder " + String(encoderNum);
}
...
String Manager_I2C::getDevicePrefixForMQTT() const {
    // Use MAC address as unique device identifier
...
void Manager_I2C::begin() {
    ...
    Serial.printf("[I2C] Initializing I2C and %d Seesaw encoder(s)...\n", encoderCount);
    ...
    // Initialize all found encoders
    for (int i = 0; i < encoderCount; i++) {
        uint8_t address = encoderAddresses[i];
        int encoderNum = getEncoderNumber(address);
        
        Serial.printf("[I2C] Initializing encoder #%d at address 0x%02X\n", encoderNum, address);
        
        // Create new Seesaw instance for this encoder
        seesawInstances[i] = new Adafruit_seesaw(wire);
        
        if (!seesawInstances[i]->begin(address)) {
            Serial.printf("[ERROR][Manager_I2C] Couldn't find seesaw encoder #%d on I2C address 0x%02X\n", encoderNum, address);
            delete seesawInstances[i];
            seesawInstances[i] = nullptr;
            continue;
        }
        
        uint32_t version = ((seesawInstances[i]->getVersion() >> 16) & 0xFFFF);
        if (version != 4991) {
            Serial.printf("[ERROR][Manager_I2C] Wrong firmware loaded on encoder #%d?\n", encoderNum);
            delete seesawInstances[i];
            seesawInstances[i] = nullptr;
            continue;
        }
        
        Serial.printf("[I2C] Found Product 4991 (Encoder #%d)\n", encoderNum);
        
        // Get starting position
        encoderPositions[i] = seesawInstances[i]->getEncoderPosition();
        
        // Turn on the encoder
        seesawInstances[i]->pinMode(24, INPUT_PULLUP);
        seesawInstances[i]->setGPIOInterrupts((uint32_t)1 << 24, 1);
        seesawInstances[i]->enableEncoderInterrupt();
    }
    ...
    Serial.printf("[I2C] Successfully initialized %d Seesaw encoder(s)\n", encoderCount);
}

void Manager_I2C::runLoop() {
    if (!initialized) return;
    
    // Process all initialized encoders
    for (int i = 0; i < encoderCount; i++) {
        if (seesawInstances[i] == nullptr) continue;
        
        uint8_t address = encoderAddresses[i];
        int encoderNum = getEncoderNumber(address);
        
        // Read encoder position
        int32_t new_position = seesawInstances[i]->getEncoderPosition();
        if (encoderPositions[i] != new_position) {
            Serial.printf("[I2C] Encoder #%d position: %d\n", encoderNum, new_position);
            
            if (mqttManager) {
                String topic = "JunctionRelay/" + getDevicePrefixForMQTT() + "/encoder" + String(encoderNum) + "/position";
                String payload = "{\"value\":" + String(new_position) + 
                                ",\"delta\":" + String(new_position - encoderPositions[i]) + 
                                ",\"encoder\":" + String(encoderNum) + "}";
                mqttManager->publish(topic.c_str(), payload);
            }
            
            encoderPositions[i] = new_position;
        }
        
        // Read button state and detect changes
        bool currentButtonState = !seesawInstances[i]->digitalRead(24); // true = pressed, false = released
        
        if (currentButtonState != buttonStates[i]) {
            if (currentButtonState) {
                Serial.printf("[I2C] Encoder #%d button pressed!\n", encoderNum);
            } else {
                Serial.printf("[I2C] Encoder #%d button released!\n", encoderNum);
            }
            
            if (mqttManager) {
                String topic = "JunctionRelay/" + getDevicePrefixForMQTT() + "/encoder" + String(encoderNum) + "/button";
                String payload = "{\"value\":" + String(currentButtonState ? 1 : 0) + 
                                ",\"state\":\"" + String(currentButtonState ? "pressed" : "released") + 
                                "\",\"encoder\":" + String(encoderNum) + "}";
                mqttManager->publish(topic.c_str(), payload);
            }
