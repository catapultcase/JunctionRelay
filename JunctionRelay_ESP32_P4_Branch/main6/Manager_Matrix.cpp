...
void Manager_Matrix::updateSensorData(const JsonDocument& sensorDoc) {
    Serial.println("[Manager_Matrix] updateSensorData() called");
    
    if (!initialized || !matrix) {
        Serial.println("[Manager_Matrix] ERROR: Not initialized or no matrix");
        return;
    }
    
    if (!acquireMatrix("updateSensorData")) {
        Serial.println("[Manager_Matrix] ERROR: Could not acquire matrix mutex");
        return;
    }
    
    Serial.println("[Manager_Matrix] Setting display mode and clearing screen");
    setDisplayMode(MODE_SENSOR_DATA);
    matrix->fillScreen(0);
    
    if (!sensorDoc.containsKey("sensors")) {
        Serial.println("[Manager_Matrix] ERROR: No 'sensors' key in JSON");
        matrix->show();
        releaseMatrix();
        return;
    }
    
    Serial.println("[Manager_Matrix] Found 'sensors' key in JSON");
    
    JsonObjectConst sensors = sensorDoc["sensors"];
    int sensorCount = 0;
    int scrollingSensorCount = 0;  // Separate counter for scrolling sensors
    const int maxSensors = 4; // Limit to 4 rows for 32-pixel height (8 pixels per row)
    const int rowHeight = 8; // 8 pixels per row
    
    Serial.printf("[Manager_Matrix] Processing sensors object with %d items\n", sensors.size());
    ...
    for (JsonPairConst kv : sensors) {
        Serial.printf("[Manager_Matrix] Processing sensor: %s\n", kv.key().c_str());
        
        if (sensorCount >= maxSensors) {
            Serial.printf("[Manager_Matrix] Limiting to %d sensors to fit display rows\n", maxSensors);
            break;
        }
        
        // Each sensor key maps to an array of readings
        JsonArrayConst sensorArray = kv.value();
        if (sensorArray.size() == 0) {
            Serial.printf("[Manager_Matrix] Skipping %s - empty array\n", kv.key().c_str());
            continue;
        }
        
        Serial.printf("[Manager_Matrix] Sensor %s has %d readings\n", kv.key().c_str(), sensorArray.size());
        
        // Get the first reading from this sensor
        JsonObjectConst sensorReading = sensorArray[0];
        if (!sensorReading.containsKey("Value") || !sensorReading.containsKey("Unit")) {
            Serial.printf("[Manager_Matrix] Skipping %s - missing Value or Unit\n", kv.key().c_str());
            continue;
        }
        
        // Calculate position based on sequential row assignment
        int x = 0; // Always start at left edge
        int y = sensorCount * rowHeight; // Sequential rows: 0, 8, 16, 24
        ...
        // Create display text from sensor name, value and unit
        const char* sensorName = kv.key().c_str();
        const char* value = sensorReading["Value"];
        const char* unit = sensorReading["Unit"];
        
        // Format: "CPU:12.1%"
        char displayText[64];
        snprintf(displayText, sizeof(displayText), "%s:%s%s", sensorName, value, unit);
        
        Serial.printf("[Manager_Matrix] Sensor %d: %s at (%d,%d)\n", sensorCount, displayText, x, y);
