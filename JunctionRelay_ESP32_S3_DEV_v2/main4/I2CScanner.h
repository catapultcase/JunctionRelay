...
public:
    ...
    static String scanAndConfigureDevices(TwoWire& wireInterface, StaticJsonDocument<2048>& doc, const String& devicePrefix) {
        ...
        for (int address = 1; address < 127; address++) {
            ...
            if (error == 0) {
                ...
                if (deviceInfo.deviceType != "Unknown") {
                    ...
                    else if (deviceInfo.deviceType == "Charlieplex") {
                        configureCharlieDisplayDevice(screens, (uint8_t)address);
                        foundCharlieplex = true;
                        Serial.printf("[DEBUG][I2CScanner] Configured Charlieplex display at 0x%02X\n", address);
                    }
                    Serial.flush();
                } else {
        ...
        doc["FoundCharlieplex"] = foundCharlieplex;
...
private:
    static I2CDeviceInfo identifyDevice(uint8_t address) {
        ...
        // QuadDisplay addresses: 0x70-0x73
        ...
        // Charlieplex addresses: 0x74-0x77
        else if (address >= 0x74 && address <= 0x77) {
            info.deviceType = "Charlieplex";
            info.displayName = "Adafruit Charlieplex Display";
            info.requiresManager = false;
            info.isDisplay = true;
        }
        else {
    ...
    static void configureCharlieDisplayDevice(JsonArray& screens, uint8_t address) {
        Serial.printf("[DEBUG][I2CScanner] Configuring Charlieplex Display device at 0x%02X\n", address);
