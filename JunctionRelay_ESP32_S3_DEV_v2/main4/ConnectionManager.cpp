    ...
    streamPayloadBuffer(nullptr)
{
    ...
    // Initialize stream prefix buffer
...
void ConnectionManager::init() {
    ...
    if (Serial) {
        Serial.setRxBufferSize(4096);  // Set large buffer for native USB
        Serial.println("[USB] Native USB CDC initialized");
        Serial.printf("[USB] Initial heap: %d, Buffer size: %d\n", initialHeap, USB_BUFFER_SIZE);
    } else {
        Serial.println("[USB] Native USB CDC not available, continuing without USB");
    }
...
void ConnectionManager::handleSerialData() {
    // Handle USB Serial (Serial Monitor) - Native USB CDC
    if (Serial.available() > 0) {
        handleNativeUSBData();
    }
    
    // Handle UART Serial (TX/RX pins) - keep existing behavior
...
void ConnectionManager::handleNativeUSBData() {
    ...
    // Native USB data reading - more efficient than UART
    while (Serial.available() && bytesRead < (USB_BUFFER_SIZE - 1)) {
        // BOUNDS CHECK: Verify we're not about to overflow
        if (bytesRead >= USB_BUFFER_SIZE) {
            Serial.print("USB BUFFER OVERFLOW PREVENTED at index: ");
            Serial.println(bytesRead);
            return; // Don't restart in main app, just exit
        }
        
        uint8_t b = Serial.read();
        usbSerialBuffer[bytesRead++] = b;
        
        // PROGRESS DEBUG: Show every 200 bytes for large payloads (less frequent for native USB)
        if (bytesRead % 200 == 0 && bytesRead > 0) {
            Serial.print("USB READING: ");
            Serial.print(bytesRead);
            Serial.println(" bytes...");
        }
        
        // Less frequent yielding needed for native USB (more efficient)
        if (bytesRead % 100 == 0) {
            yield();
        }
    }
    ...
    if (bytesRead > 0) {
        Serial.printf("[USB] Read %d bytes via Native USB CDC\n", bytesRead);
...
void ConnectionManager::checkMemoryHealth(const char* location) {
    ...
    if (currentHeap < 20000) {
        ...
        Serial.print(", used: ");
        Serial.print(initialHeap - currentHeap);
        Serial.println(")");
    }
    
    // MEMORY PROTECTION - log warning if heap gets low
    if (currentHeap < 15000) {
        Serial.println("*** LOW MEMORY WARNING ***");
        // Don't restart in main app, just warn
    }
}
...
void ConnectionManager::handleIncomingDataChunkPrefix(uint8_t *data, size_t len) {
    ...
    Serial.printf("[DEBUG] readingLength=%s, bytesRead=%d, len=%d, first_char='%c' (0x%02X)\n", 
                 streamReadingLength ? "true" : "false", streamBytesRead, len, 
                 (data[0] >= 32 && data[0] <= 126) ? data[0] : '?', data[0]);

    // Print first 50 characters to see the full content
    Serial.print("[DEBUG] First 50 chars: ");
    for (int i = 0; i < min(50, (int)len); i++) {
    ...
    // NEW: Enhanced payload detection for 4 types at start of new message
    if (streamReadingLength && streamBytesRead == 0 && len > 0) {
        // Check if first character is '{' - Raw JSON (Type 1)
        if (data[0] == '{') {
        ...
        // Check for gzip magic bytes - Raw Gzip (Type 3)
    ...
    // EXISTING: Stage 1: Extract the 8-byte prefix (LLLLTTRR format)
    if (streamReadingLength) {
        ...
        if (streamBytesRead >= 8) {
            ...
            // For now, use length hint as payload length
            // TODO: Implement auto-detection when length hint is 0000
            if (lengthHintValue > 0) {
                streamPayloadLength = lengthHintValue;
            } else {
                // Fallback: assume reasonable default for auto-detection
...
// Replace the existing resetStreamState method
...
// Helper method to decompress and process gzip data
