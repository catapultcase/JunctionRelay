    ...
    public class Service_Stream_Manager_HTTP
    {
            ...
            int linkId = 0)
        {
            ...
            using (var scope = _scopeFactory.CreateScope())
            {
                ...
                {
                    protocolString += " (MJPEG Streaming)";
                }
                ...
                {
                    Console.WriteLine($"[SERVICE_STREAM_MANAGER_HTTP] Starting in Streaming mode for {screenKey}");

                    var frameLayoutId = screenOverride?.FrameLayoutId ?? screen.FrameLayoutId;
                    if (!frameLayoutId.HasValue)
                    {
                        Console.WriteLine($"[SERVICE_STREAM_MANAGER_HTTP] No frame layout ID for streaming config");
                        info.Dispose();
                        _streamingTokens.TryRemove(screen.Id, out _);
                        return;
                    }
                    ...
                    // Get virtual device ID (created earlier by CreateBlitModeVirtualScreenAsync)
                    var virtualStreamManager = scope.ServiceProvider.GetRequiredService<Service_Stream_Manager_Virtual>();
                    var virtualDeviceId = virtualStreamManager.GetBlitModeVirtualDeviceId(screen.Id);
                    if (!virtualDeviceId.HasValue)
                    {
                        Console.WriteLine($"[SERVICE_STREAM_MANAGER_HTTP] No virtual device found for screen {screen.Id}");
                        info.Dispose();
                        _streamingTokens.TryRemove(screen.Id, out _);
                        return;
                    }

                    // Get server base URL - use IServer feature to get actual bound address
                    var server = scope.ServiceProvider.GetRequiredService<IServer>();
                    var addresses = server.Features.Get<IServerAddressesFeature>()?.Addresses;
                    string baseUrl = "http://localhost:7180"; // Fallback
                    if (addresses != null && addresses.Any())
                    {
                        var address = addresses.First();
                        baseUrl = address.Replace("[::]", "0.0.0.0").Replace("*", "0.0.0.0");
                    }
                    ...
                    var streamConfig = new
                    {
                        ...
                        screenId = screenKey,
                        mode = "streaming",
                        streamUrl = $"{baseUrl}/device/{virtualDeviceId}/stream",
                        frameFormat = "MJPEG",
                        frameWidth = frameLayout.Width,
                        frameHeight = frameLayout.Height,
                        jpegQuality = junction.StreamingJpegQuality,
                        targetFps = screenOverride?.StreamingFps ?? 30, // Per-screen FPS (default: 30)
                        description = "MJPEG stream - device should fetch from streamUrl"
                    };

                    string configJson = JsonSerializer.Serialize(streamConfig);
                    var configBytes = Encoding.UTF8.GetBytes(configJson);

                    var result = await httpSender.SendPayloadWithHealthAsync(configBytes);
                    result.PayloadType = "Stream Config";
                    info.Health.UpdateHealth(result);

                    if (!result.Success)
                    {
                        Console.WriteLine($"[SERVICE_STREAM_MANAGER_HTTP] Failed to send streaming config.");
                        info.Dispose();
                        _streamingTokens.TryRemove(screen.Id, out _);
                        return;
                    }

                    info.ConfigPayloadPrefix = "STREAM_CONFIG";
                    info.UpdateConfigPayload(configJson);

                    Console.WriteLine($"[SERVICE_STREAM_MANAGER_HTTP] Streaming config sent to {device.Name}: {streamConfig.streamUrl}");

                    // Streaming mode does NOT enter the sensor loop - config sent once, device fetches independently
                    // Return early to skip the sensor polling loop
                    Console.WriteLine($"[SERVICE_STREAM_MANAGER_HTTP] Streaming mode initialization complete for {device.Name}/{screenKey} - no polling loop needed");
                    return;
                }
                else if (isCompositeMode)
                {
            ...
            _ = Task.Run(async () =>
            {
                ...
                while (!cts.Token.IsCancellationRequested)
                {
                    ...
                    catch (ObjectDisposedException ex) when (ex.ObjectName == "CancellationTokenSource")
                    {
                        ...
                        Console.WriteLine($"[SERVICE_STREAM_MANAGER_HTTP] CTS disposed during streaming loop - shutting down gracefully");
                        break;
                    }
                    catch (Exception ex)
                    {
