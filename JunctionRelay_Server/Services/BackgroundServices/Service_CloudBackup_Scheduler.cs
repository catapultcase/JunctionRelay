...
namespace JunctionRelayServer.Services.BackgroundServices
{
    public class Service_CloudBackup_Scheduler : BackgroundService
    {
        ...
        private readonly Service_CloudBackupScheduler_Logger _logger;
        ...
        public Service_CloudBackup_Scheduler(
            ...
            Service_CloudBackupScheduler_Logger logger)
        {
        ...
        private async Task CheckAndPerformBackupAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(token))
                {
                    ...
                    if (_lastBackupCheck.HasValue)
                    {
                        ...
                        _logger.LogBackupSkipped("No valid cloud authentication");
                ...
                if (!statusResponse.Success)
                {
                    ...
                    _logger.LogBackupFailed("API Call - Get Status", statusResponse.ErrorMessage ?? "Unknown error");
                    return;
                }
                ...
                var hasLicense = statusData.GetProperty("hasValidLicense").GetBoolean();
                if (!hasLicense)
                {
                    ...
                    if (_lastBackupCheck.HasValue)
                    {
                        ...
                        _logger.LogBackupSkipped("No valid Pro license");
                        _lastBackupCheck = null;
                    }
                    return;
                }
                ...
                var backupsEnabled = settings.GetProperty("enabled").GetBoolean();
                if (!backupsEnabled)
                {
                    ...
                    if (_lastBackupCheck.HasValue)
                    {
                        ...
                        _logger.LogBackupSkipped("Cloud backups disabled in settings");
                        _lastBackupCheck = null;
                    }
                    return;
                }

                // Extract backup timing info for logging
                ...
                double? hoursSinceLastBackup = null;
                if (!string.IsNullOrEmpty(lastBackupStr))
                {
                    var lastBackup = DateTime.Parse(lastBackupStr);
                    hoursSinceLastBackup = (DateTime.UtcNow - lastBackup).TotalHours;
                }

                // Check if it's time for a backup
                var shouldBackup = ShouldPerformBackup(settings, out var backupDueReason);

                // Log every check with detailed information
                _logger.LogSchedulerCheck(
                    hasAuth: true,
                    hasLicense: hasLicense,
                    backupsEnabled: backupsEnabled,
                    lastBackupUtc: lastBackupStr,
                    frequency: frequency,
                    backupDue: shouldBackup,
                    hoursSinceLastBackup: hoursSinceLastBackup,
                    skipReason: shouldBackup ? null : backupDueReason
                );
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogBackupFailed("CheckAndPerformBackup", ex.Message);
            }
        }

        private bool ShouldPerformBackup(JsonElement settings, out string reason)
        {
            reason = string.Empty;
            try
            {
                ...
                if (shouldBackup)
                {
                    ...
                    reason = $"Backup interval reached (Last backup: {timeSinceLastBackup.TotalHours:F1}h ago)";
                }
                else
                {
                    reason = $"Not due yet (Last backup: {timeSinceLastBackup.TotalHours:F1}h ago, Frequency: {frequency})";
                }
            ...
            catch (Exception ex)
            {
                ...
                reason = $"Error determining schedule: {ex.Message}";
                return false;
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                // Log backup trigger
                _logger.LogBackupTriggered(frequency, includeKeys, includeIdentity, includeFrameEngine);
                ...
                if (!backupResult.Success || backupResult.BackupData == null)
                {
                    ...
                    _logger.LogBackupFailed("Create Backup", backupResult.ErrorMessage ?? "Unknown error");
                    return false;
                }
                ...
                if (!requestUploadResult.Success)
                {
                    ...
                    _logger.LogBackupFailed("Request Upload URL", requestUploadResult.ErrorMessage ?? "Unknown error");
                    return false;
                }

                var uploadResponse = JsonSerializer.Deserialize<JsonElement>(requestUploadResult.Data!.ToString()!);
                backupId = uploadResponse.GetProperty("backupId").GetString();
                ...
                if (!uploadSuccess)
                {
                    ...
                    _logger.LogBackupFailed("Upload to S3", "S3 upload failed", backupId);
                    return false;
                }
                ...
                _logger.LogBackupCompleted(
                    backupId: backupId ?? "unknown",
                    filename: backupResult.Filename ?? "unknown.zip",
                    sizeBytes: backupResult.BackupData.Length,
                    uploadDurationMs: duration,
                    hasManifest: backupResult.Manifest != null
                );
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogBackupFailed("Perform Backup", ex.Message, backupId);
                return false;
            }
        }
