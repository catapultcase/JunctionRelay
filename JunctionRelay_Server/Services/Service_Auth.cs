...
namespace JunctionRelayServer.Services
{
    public class Service_Auth : IService_Auth
    {
        ...
        public Service_Auth(IDatabaseConnectionFactory dbFactory, ISecretsService secretsService)
        {
        ...
        public async Task<IEnumerable<Model_AuthUser>> GetAllUsersAsync()
        {
            ...
            {
                using var connection = _dbFactory.CreateConnection();
                var users = await connection.QueryAsync<Model_AuthUser>(
        ...
        {
            using var connection = _dbFactory.CreateConnection();
            var count = await connection.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM AuthUsers WHERE IsActive = 1");
            return count > 0;
        }
        ...
        public async Task RemoveUserAsync(string username)
        {
            try
            {
                ...
                using var connection = _dbFactory.CreateConnection();
                var result = await connection.ExecuteAsync(sql, new { Username = username });
        ...
        {
            using var connection = _dbFactory.CreateConnection();
            var user = await connection.QuerySingleOrDefaultAsync<Model_AuthUser>(
