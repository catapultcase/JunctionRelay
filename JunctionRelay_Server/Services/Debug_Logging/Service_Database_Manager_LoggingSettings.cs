    ...
    public class Service_Database_Manager_LoggingSettings
    {
        ...
        public async Task<Model_LoggingSettings> UpsertAsync(Model_LoggingSettings settings)
        {
            ...
            else
            {
                ...
                var id = await _db.QuerySingleAsync<int>(@"
                    INSERT INTO LoggingSettings (Category, Enabled, IsEventDriven, LogIntervalMinutes, Description, LastLoggedAt,
                        MaxLogRetentionDays, MaxLogFileSizeMB, AutoCleanupEnabled, LastCleanupAt, CreatedAt, UpdatedAt)
                    VALUES (@Category, @Enabled, @IsEventDriven, @LogIntervalMinutes, @Description, @LastLoggedAt,
                        @MaxLogRetentionDays, @MaxLogFileSizeMB, @AutoCleanupEnabled, @LastCleanupAt, @CreatedAt, @UpdatedAt);
        ...
        public async Task InitializeDefaultsAsync()
        {
            var defaults = new List<Model_LoggingSettings>
            {
                ...
                new Model_LoggingSettings
                {
                    Category = "CloudBackupScheduler",
                    Enabled = false,
                    IsEventDriven = true,
                    LogIntervalMinutes = 0,
                    Description = "Cloud backup scheduler checks, backup triggers, and upload operations"
                },
                new Model_LoggingSettings
                {
                    Category = "JunctionStartup",
                    Enabled = false,
                    IsEventDriven = true,
                    LogIntervalMinutes = 0,
                    Description = "Junction and service startup sequence logging (parallel/sequential mode, timing, success/failure tracking)"
                }
            };
