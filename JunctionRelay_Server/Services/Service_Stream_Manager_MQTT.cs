...
namespace JunctionRelayServer.Services
{
    public class Service_Stream_Manager_MQTT
    {
        ...
        {
            var subDb = new Service_Database_Manager_MQTT_Subscriptions(_dbFactory);
                ...
                while (!cts.Token.IsCancellationRequested)
                {
                    foreach (var sensor in assignedSensors)
                    {
                        ...
                        catch (ObjectDisposedException ex) when (ex.ObjectName == "CancellationTokenSource")
                        {
                            ...
                            Console.WriteLine($"[SERVICE_STREAM_MANAGER_MQTT] CTS disposed during streaming loop - shutting down gracefully");
                            break;
                        }
                        catch (Exception ex)
                        {
