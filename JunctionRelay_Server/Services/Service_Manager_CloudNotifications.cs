...
namespace JunctionRelayServer.Services
{
    public class Service_Manager_CloudNotifications
    {
        ...
        // Register device for health monitoring (enables notifications)
        public async Task<bool> RegisterForHealthMonitoringAsync(string cloudToken, string deviceId, string deviceName)
        {
            try
            {
                var cloudApiUrl = _configuration["JunctionRelayCloud:ApiUrl"];
                ...
                var registerUrl = $"{cloudApiUrl}/cloud/local-devices/register-health";
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    var errorContent = await response.Content.ReadAsStringAsync();
                    Console.WriteLine($"Failed to register device for health monitoring. Status: {response.StatusCode}, Content: {errorContent}");

                    // Handle "already registered" as success
                    if (errorContent.Contains("already registered"))
                    {
                        Console.WriteLine($"Device {deviceId} already registered for health monitoring - treating as success");
                        return true;
                    }
                ...
                var responseContent = await response.Content.ReadAsStringAsync();
                var cloudResponse = JsonSerializer.Deserialize<CloudNotificationRegistrationResponse>(responseContent, new JsonSerializerOptions
                {
                    PropertyNameCaseInsensitive = true
                });
            ...
            {
                Console.WriteLine($"Error registering device for health monitoring: {ex.Message}");
                return false;
            }
        }

        // Register device for full sync (enables notifications)
        public async Task<bool> RegisterForFullSyncAsync(string cloudToken, string deviceId, string deviceName)
        {
            try
            {
                var cloudApiUrl = _configuration["JunctionRelayCloud:ApiUrl"];
                ...
                var registerUrl = $"{cloudApiUrl}/cloud/local-devices/register-sync";
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    var errorContent = await response.Content.ReadAsStringAsync();
                    Console.WriteLine($"Failed to register device for full sync. Status: {response.StatusCode}, Content: {errorContent}");
                    return false;
                }
            ...
            {
                Console.WriteLine($"Error registering device for full sync: {ex.Message}");
                return false;
            }
        }

        // Unregister device from notifications
        public async Task<bool> UnregisterFromNotificationsAsync(string cloudToken, string deviceId)
        {
            try
            {
                var cloudApiUrl = _configuration["JunctionRelayCloud:ApiUrl"];
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    var errorContent = await response.Content.ReadAsStringAsync();
                    Console.WriteLine($"Failed to unregister device from notifications. Status: {response.StatusCode}, Content: {errorContent}");
                    ...
                    {
                        Console.WriteLine($"Device {deviceId} was not registered - treating unregister as success");
                        return true;
                    }
            ...
            {
                Console.WriteLine($"Error unregistering device from notifications: {ex.Message}");
                return false;
            }
        }
    }

    public class CloudNotificationRegistrationResponse
    {
