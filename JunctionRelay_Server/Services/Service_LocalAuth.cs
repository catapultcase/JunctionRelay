...
namespace JunctionRelayServer.Services
{
    public class Service_LocalAuth : ILocalAuthService
    {
        ...
        public Task<IActionResult> GetCurrentUserAsync(HttpContext httpContext)
        {
            if (!httpContext.User.Identity?.IsAuthenticated ?? true)
            {
                return Task.FromResult<IActionResult>(new UnauthorizedObjectResult(new { message = "Not authenticated" }));
            }

            var username = httpContext.User.Identity?.Name ?? string.Empty;
            return Task.FromResult<IActionResult>(new OkObjectResult(new { username, authMode = "local" }));
        }

        public Task<IActionResult> ValidateTokenAsync(HttpContext httpContext)
        {
        ...
        public Task<IActionResult> LogoutAsync(HttpContext httpContext)
        {
            var username = httpContext.User.Identity?.Name ?? "unknown";
            Console.WriteLine($"Local user logged out: {username}");
            return Task.FromResult<IActionResult>(new OkObjectResult(new { message = "Logged out successfully", authMode = "local" }));
        }
        ...
        public Task<IActionResult> GetTokenInfoAsync(HttpContext httpContext)
        {
