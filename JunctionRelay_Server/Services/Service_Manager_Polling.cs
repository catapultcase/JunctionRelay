...
public class Service_Manager_Polling
{
    ...
    )
    {
        ...
        lock (_pollerLock)
        {
            ...
            {
                Console.WriteLine($"[SERVICE_MANAGER_POLLING] Creating new poller for key {key} at {pollRateMs}...");
    ...
    )
    {
        ...
        if (_activePollers.TryAdd(key, newPoller))
        {
            Task.Run(() => RunPollLoop(newPoller, cts.Token));
        }
        else
        {
            newPoller.TokenSource.Cancel();
            newPoller.TokenSource.Dispose();
            Console.WriteLine($"[POLLING] Prevented duplicate poller creation for key: {key}");
        }
    }
