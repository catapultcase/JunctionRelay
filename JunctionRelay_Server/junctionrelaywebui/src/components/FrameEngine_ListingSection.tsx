...
const defaultFrameEngineColumns: FrameEngineColumn[] = [
    ...
    { field: "mode", label: "Mode", align: "center", sortable: true },
...
const FrameLayoutTableRow = memo(({
    ...
    const getFrameLayoutCell = useCallback((field: string) => {
        switch (field) {
            ...
            case "mode":
                const modeLabel = frameLayout.layoutType === 'PIXEL_MODE' ? "Pixel" : "Composite";
                const modeColor = frameLayout.layoutType === 'PIXEL_MODE' ? "warning" : "primary";
                return (
                    <Chip
                        label={modeLabel}
                        color={modeColor}
