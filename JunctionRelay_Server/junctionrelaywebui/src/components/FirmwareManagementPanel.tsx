...
const FirmwareManagementPanel: React.FC<FirmwareManagementPanelProps> = ({
    ...
    const verifyFirmware = useCallback(async () => {
        if (!isJunctionRelayDevice) {
            showSnackbar("Firmware verification is only available for JunctionRelay devices", "error");
            return;
        }
    ...
    const checkForUpdates = useCallback(async (forceCheck: boolean = false) => {
        if (!isJunctionRelayDevice) {
            showSnackbar("Firmware updates are only available for JunctionRelay devices", "error");
            return;
        }
                ...
                <Box sx={{ mb: 3 }}>
                    ...
                    {!isJunctionRelayDevice && (
                        <Alert severity="info" sx={{ mb: 2 }}>
                            <Typography variant="body2">
                                Firmware management is only available for JunctionRelay devices
                            </Typography>
                        </Alert>
                    )}
