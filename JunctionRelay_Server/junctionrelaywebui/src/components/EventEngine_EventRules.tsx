...
const EventEngineEventRules: React.FC<EventEngineEventRulesProps> = ({
    ...
    const handleAddEvent = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/eventrules', {
                method: 'POST',
            ...
            showSnackbar("Event rule created successfully", "success");
            navigate(`/configure-eventrule/${savedRule.id}`);
        } catch (error) {
    ...
    useEffect(() => {
        ...
        window.addEventListener('bottom-action-add-event', handleBottomAddEvent);
        ...
        return () => {
            window.removeEventListener('bottom-action-add-event', handleBottomAddEvent);
                ...
                <Box>
                    ...
                    {!isMobile && (
                        <Box sx={{ display: "flex", gap: 2, mb: 2, flexWrap: 'wrap' }}>
                            ...
                            >
                                {isCreating ? 'Creating...' : 'Add Event Rule'}
                            </Button>
                        </Box>
                    )}
                            ...
                            {sortedEvents.length > 0 ? (
                                ...
                                <Paper sx={{ p: 3, textAlign: 'center', gridColumn: '1 / -1' }}>
                                    ...
                                    <Button
                                        ...
                                        sx={{ mt: 2 }}
                                    >
                                        {isCreating ? 'Creating...' : 'Add Your First Event'}
                                    </Button>
                                </Paper>
                            )}
                        </Box>
