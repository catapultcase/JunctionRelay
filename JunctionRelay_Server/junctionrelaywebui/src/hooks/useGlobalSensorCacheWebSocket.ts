...
export const useGlobalSensorCacheWebSocket = (options: WebSocketHookOptions = {}) => {
    ...
    const [currentPollRate, setCurrentPollRate] = useState<number>(() => {
        try {
            ...
            if (stored) {
                ...
                if (!isNaN(parsed) && parsed > 0) {
                    console.debug('[WebSocket] Loaded poll rate from storage:', parsed);
                    return parsed;
                }
            }
        } catch (e) {
        ...
        console.debug('[WebSocket] Using default poll rate:', defaultPollRate);
        ...
        ws.current.onmessage = (event) => {
            try {
                ...
                if (message.type === 'sensor-cache-update') {
                    console.debug('[WebSocket] Received sensor update', message.data);
                    setData(prev => ({
                        ...prev,
    ...
    const disconnect = useCallback(() => {
        if (ws.current) {
            console.debug('[WebSocket] Manual disconnect');
    ...
    const setPollRate = useCallback((rate: number) => {
        console.debug('[WebSocket] Poll rate changed to', rate);
        setCurrentPollRate(rate);

        // Persist to localStorage
        try {
            localStorage.setItem(STORAGE_KEY_POLL_RATE, rate.toString());
            console.debug('[WebSocket] Poll rate saved to localStorage');
        } catch (e) {
...
export const useMqttCacheWebSocket = (options: MqttWebSocketHookOptions = {}) => {
    ...
    const [currentPollRate, setCurrentPollRate] = useState<number>(() => {
        try {
            ...
            if (stored) {
                const parsed = parseInt(stored, 10);
                if (!isNaN(parsed) && parsed > 0) {
                    console.debug('[MQTT WebSocket] Loaded poll rate from storage:', parsed);
                    return parsed;
                }
            }
        } catch (e) {
        ...
        console.debug('[MQTT WebSocket] Using default poll rate:', defaultPollRate);
        return defaultPollRate;
    });
    ...
    const connect = useCallback(() => {
        ...
        console.debug('[MQTT WebSocket] Connecting to', wsUrl);
        ws.current = new WebSocket(wsUrl);

        ws.current.onopen = () => {
            console.debug('[MQTT WebSocket] Connected');
            setData(prev => ({ ...prev, connectionStatus: 'connected' }));
        };
        ...
        ws.current.onmessage = (event) => {
            try {
                ...
                if (message.type === 'mqtt-services-update') {
                    console.debug('[MQTT WebSocket] Received MQTT services update', message.data);
                    setData(prev => ({
                        ...prev,
                    ...
                    console.debug('[MQTT WebSocket] Received MQTT payload update', message.data);
    ...
    const disconnect = useCallback(() => {
        if (ws.current) {
            console.debug('[MQTT WebSocket] Manual disconnect');
            ws.current.close(1000, 'Manual disconnect');
            ws.current = null;
        }
        setData(prev => ({ ...prev, connectionStatus: 'disconnected' }));
    ...
    const setPollRate = useCallback((rate: number) => {
        console.debug('[MQTT WebSocket] Poll rate changed to', rate);
        setCurrentPollRate(rate);

        // Persist to localStorage
        try {
            localStorage.setItem(MQTT_STORAGE_KEY_POLL_RATE, rate.toString());
            console.debug('[MQTT WebSocket] Poll rate saved to localStorage');
        } catch (e) {
...
export const useEventSensorCacheWebSocket = (options: EventSensorWebSocketHookOptions = {}) => {
    ...
    const [currentPollRate, setCurrentPollRate] = useState<number>(() => {
        try {
            ...
            if (stored) {
                const parsed = parseInt(stored, 10);
                if (!isNaN(parsed) && parsed > 0) {
                    console.debug('[Event Sensor WebSocket] Loaded poll rate from storage:', parsed);
                    return parsed;
                }
            }
        } catch (e) {
        ...
        console.debug('[Event Sensor WebSocket] Using default poll rate:', defaultPollRate);
        return defaultPollRate;
    });
    ...
    const connect = useCallback(() => {
        ...
        console.debug('[Event Sensor WebSocket] Connecting to', wsUrl);
        ws.current = new WebSocket(wsUrl);

        ws.current.onopen = () => {
            console.debug('[Event Sensor WebSocket] Connected');
            setData(prev => ({ ...prev, connectionStatus: 'connected' }));
        };
        ...
        ws.current.onmessage = (event) => {
            try {
                ...
                if (message.type === 'event-sensors-update') {
                    console.debug('[Event Sensor WebSocket] Received event sensors update', message.data);
                    setData(prev => ({
                        ...prev,
    ...
    const disconnect = useCallback(() => {
        if (ws.current) {
            console.debug('[Event Sensor WebSocket] Manual disconnect');
            ws.current.close(1000, 'Manual disconnect');
            ws.current = null;
        }
        setData(prev => ({ ...prev, connectionStatus: 'disconnected' }));
    ...
    const setPollRate = useCallback((rate: number) => {
        console.debug('[Event Sensor WebSocket] Poll rate changed to', rate);
        setCurrentPollRate(rate);

        // Persist to localStorage
        try {
            localStorage.setItem(EVENT_SENSOR_STORAGE_KEY_POLL_RATE, rate.toString());
            console.debug('[Event Sensor WebSocket] Poll rate saved to localStorage');
        } catch (e) {
...
export const useEventRulesCacheWebSocket = (options: EventRulesWebSocketHookOptions = {}) => {
    ...
    const [currentPollRate, setCurrentPollRate] = useState<number>(() => {
        try {
            ...
            if (stored) {
                const parsed = parseInt(stored, 10);
                if (!isNaN(parsed) && parsed > 0) {
                    console.debug('[Event Rules WebSocket] Loaded poll rate from storage:', parsed);
                    return parsed;
                }
            }
        } catch (e) {
        ...
        console.debug('[Event Rules WebSocket] Using default poll rate:', defaultPollRate);
        return defaultPollRate;
    });
    ...
    const connect = useCallback(() => {
        ...
        console.debug('[Event Rules WebSocket] Connecting to', wsUrl);
        ws.current = new WebSocket(wsUrl);

        ws.current.onopen = () => {
            console.debug('[Event Rules WebSocket] Connected');
            setData(prev => ({ ...prev, connectionStatus: 'connected' }));
        };
        ...
        ws.current.onmessage = (event) => {
            try {
                ...
                if (message.type === 'event-rules-update') {
                    console.debug('[Event Rules WebSocket] Received event rules update', message.data);
                    setData(prev => ({
                        ...prev,
    ...
    const disconnect = useCallback(() => {
        if (ws.current) {
            console.debug('[Event Rules WebSocket] Manual disconnect');
            ws.current.close(1000, 'Manual disconnect');
            ws.current = null;
        }
        setData(prev => ({ ...prev, connectionStatus: 'disconnected' }));
    ...
    const setPollRate = useCallback((rate: number) => {
        console.debug('[Event Rules WebSocket] Poll rate changed to', rate);
        setCurrentPollRate(rate);

        // Persist to localStorage
        try {
            localStorage.setItem(EVENT_RULES_STORAGE_KEY_POLL_RATE, rate.toString());
            console.debug('[Event Rules WebSocket] Poll rate saved to localStorage');
        } catch (e) {
