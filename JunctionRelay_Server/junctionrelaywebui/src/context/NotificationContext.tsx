...
export interface Notification {
    ...
    structuredContent?: StructuredContent; // for multi-line notifications
}
...
export const NotificationProvider: React.FC<NotificationProviderProps> = ({ children }) => {
    ...
    const showNotification = useCallback((notification: Omit<Notification, 'id' | 'timestamp'>) => {
        ...
        if (newNotification.duration && newNotification.duration > 0 && !newNotification.persistent) {
            console.log(`[Notification] Setting auto-dismiss timer for ${newNotification.duration}ms, ID: ${id}`);
            setTimeout(() => {
                console.log(`[Notification] Auto-dismissing notification ID: ${id}`);
                dismissNotification(id);
            }, newNotification.duration);
        } else {
            console.log(`[Notification] NOT setting auto-dismiss because:`, {
                duration: newNotification.duration,
                persistent: newNotification.persistent,
                durationCheck: newNotification.duration && newNotification.duration > 0,
                persistentCheck: !newNotification.persistent
            });
        }
    }, [isEnabled, getSuccessDuration, getErrorDuration, getMaxConcurrent]);
