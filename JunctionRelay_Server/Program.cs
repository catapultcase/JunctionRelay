...
builder.Services.AddTransient<DataCollector_Steam>();
...
builder.Services.AddTransient<DataCollector_VirtualDevice>();
    ...
    var creatorMap = new Dictionary<string, Func<Model_Collector, IDataCollector>>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "Steam", c => { var i = provider.GetRequiredService<DataCollector_Steam>(); i.ApplyConfiguration(c); return i; } },
            ...
            { "VirtualDevice", c => { var i = provider.GetRequiredService<DataCollector_VirtualDevice>(); i.ApplyConfiguration(c); return i; } }
        };
...
app.Lifetime.ApplicationStarted.Register(async () =>
{
    ...
    try
    {
        ...
        try
        {
            ...
            junctionStartupLogger.Initialize();
...
app.Lifetime.ApplicationStopping.Register(() =>
{
    ...
    // Stop all running junctions FIRST - this prevents stream managers from operating during shutdown
    try
    {
        var connectionManager = app.Services.GetRequiredService<Service_Manager_Connections>();
        var runningJunctionIds = connectionManager.RunningJunctions.Keys.ToList();

        if (runningJunctionIds.Count > 0)
        {
            Console.WriteLine($"[SHUTDOWN] Stopping {runningJunctionIds.Count} running junctions...");

            foreach (var junctionId in runningJunctionIds)
            {
                try
                {
                    var stopTask = connectionManager.StopJunctionAsync(junctionId, CancellationToken.None);
    ...
    {
        Console.WriteLine($"[SHUTDOWN] Error stopping junctions: {ex.Message}");
    }

    // Stop virtual streams BEFORE disposing Puppeteer to prevent race condition
    try
    {
        var virtualStreamManager = app.Services.GetRequiredService<Service_Stream_Manager_Virtual>();
        var activeStreams = virtualStreamManager.GetActiveStreams().ToList();

        if (activeStreams.Count > 0)
        {
            Console.WriteLine($"[SHUTDOWN] Stopping {activeStreams.Count} virtual streams...");

            foreach (var stream in activeStreams)
            {
                virtualStreamManager.StopStreaming(stream.ScreenId);
            }
    ...
    {
        Console.WriteLine($"[SHUTDOWN] Error stopping virtual streams: {ex.Message}");
    }

    // Dispose Puppeteer/Chrome instances to prevent orphaned Chrome_Testing processes
    try
    {
        var puppeteerService = app.Services.GetRequiredService<Service_FrameEngine_Puppeteer>();
        puppeteerService.Dispose();
        Console.WriteLine("[SHUTDOWN] Puppeteer service disposed - Chrome instances cleaned up");
    }
    catch (Exception ex)
    {
        Console.WriteLine($"[SHUTDOWN] Error disposing Puppeteer service: {ex.Message}");
    }

    // Close all unified notification broadcaster connections gracefully
    try
    {
        var unifiedNotificationBroadcaster = app.Services.GetRequiredService<Service_Unified_Notification_Broadcaster>();
        var closeTask = unifiedNotificationBroadcaster.CloseAllConnectionsAsync("Application shutdown");

        // Wait up to 2 seconds for graceful shutdown
        if (closeTask.Wait(TimeSpan.FromSeconds(2)))
        {
            Console.WriteLine("[SHUTDOWN] Unified notification broadcaster connections closed gracefully");
        }
        else
        {
            Console.WriteLine("[SHUTDOWN] Unified notification broadcaster close operation timed out after 2 seconds");
        }
    }
    catch (Exception ex)
    {
        Console.WriteLine($"[SHUTDOWN] Error closing unified notification broadcaster: {ex.Message}");
    }

    // Close all WebSocket server connections gracefully
    try
    {
        var webSocketServer = app.Services.GetRequiredService<Service_Manager_WebSocket_Server>();
        var closeTask = webSocketServer.CloseAllConnectionsAsync("Application shutdown");

        // Wait up to 3 seconds for graceful shutdown (200ms polling stop + connections close)
        if (closeTask.Wait(TimeSpan.FromSeconds(3)))
        {
            Console.WriteLine("[SHUTDOWN] WebSocket server connections closed gracefully");
        }
        else
        {
            Console.WriteLine("[SHUTDOWN] WebSocket server close operation timed out after 3 seconds");
        }
    }
    catch (Exception ex)
    {
        Console.WriteLine($"[SHUTDOWN] Error closing WebSocket server: {ex.Message}");
    }
});
