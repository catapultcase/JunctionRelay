...
namespace JunctionRelayServer.Controllers
{
    ...
    public class Controller_FrameEngine_Layouts : ControllerBase
    {
        ...
        public async Task<ActionResult<CreateFrameLayoutResponse>> CreateFrameLayout([FromBody] CreateFrameLayoutRequest request)
        {
            try
            {
                ...
                var layoutType = string.IsNullOrWhiteSpace(request.LayoutType) ? "PRE_RENDERED_IMAGE" : request.LayoutType.ToUpperInvariant();

                // Set default dimensions based on layout type
                int defaultWidth = layoutType == "PIXEL_MODE" ? 64 : 800;
                int defaultHeight = layoutType == "PIXEL_MODE" ? 64 : 600;
                ...
                var newFrameLayout = new Model_Frame_Layout
                {
                    ...
                    BackgroundLoopCompensation = (backgroundType == "video") ? request.BackgroundLoopCompensation : null,
                    BackgroundLoopCutoverMs = (backgroundType == "video") ? request.BackgroundLoopCutoverMs : null,
                    BackgroundLoopCrossfadeMs = (backgroundType == "video") ? request.BackgroundLoopCrossfadeMs : null,
        ...
        public async Task<ActionResult> UpdateFrameLayout(int id, [FromBody] UpdateFrameLayoutRequest request)
        {
            try
            {
                ...
                if (request.IsStandalone.HasValue)
                    existing.IsStandalone = request.IsStandalone.Value;
                ...
                if (backgroundType != "video")
                {
                    ...
                    existing.BackgroundLoopCompensation = null;
                    existing.BackgroundLoopCutoverMs = null;
                    existing.BackgroundLoopCrossfadeMs = null;
                }
                else
                {
                    ...
                    if (request.BackgroundLoopCompensation.HasValue)
                        existing.BackgroundLoopCompensation = request.BackgroundLoopCompensation;
                    if (request.BackgroundLoopCutoverMs.HasValue)
                        existing.BackgroundLoopCutoverMs = request.BackgroundLoopCutoverMs;
                    if (request.BackgroundLoopCrossfadeMs.HasValue)
                        existing.BackgroundLoopCrossfadeMs = request.BackgroundLoopCrossfadeMs;
                }
        ...
        [HttpGet("sensor-dictionary")]
        public ActionResult GetSensorDictionary()
        {
            try
            {
                var sensorDictionaryPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "frameengine", "sensor-dictionary.json");

                if (!System.IO.File.Exists(sensorDictionaryPath))
                {
                    return NotFound(new { message = "Sensor dictionary file not found" });
                }

                var jsonContent = System.IO.File.ReadAllText(sensorDictionaryPath);
                var sensors = JsonSerializer.Deserialize<JsonElement>(jsonContent);

                return Ok(sensors);
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { message = "Error loading sensor dictionary", error = ex.Message });
            }
        }
        ...
        private static FrameLayoutDto MapToFrameLayoutDto(Model_Frame_Layout frameLayout)
        {
            return new FrameLayoutDto
            {
                ...
                IsStandalone = frameLayout.IsStandalone,
                ...
                BackgroundLoopCompensation = frameLayout.BackgroundLoopCompensation,
                BackgroundLoopCutoverMs = frameLayout.BackgroundLoopCutoverMs,
                BackgroundLoopCrossfadeMs = frameLayout.BackgroundLoopCrossfadeMs,
            ...
            int? height)
        {
            ...
            if (string.IsNullOrWhiteSpace(layoutType))
                ...
                errors.Add($"Invalid LayoutType: {layoutType}. Valid types are: PRE_RENDERED_IMAGE, COMPOSITE_MODE, PIXEL_MODE");
        ...
        {
            var validTypes = new[] { "PRE_RENDERED_IMAGE", "COMPOSITE_MODE", "PIXEL_MODE" };
    ...
    public class FrameLayoutDto
    {
        ...
        public bool? BackgroundLoopCompensation { get; set; }
        public int? BackgroundLoopCutoverMs { get; set; }
        public int? BackgroundLoopCrossfadeMs { get; set; }
