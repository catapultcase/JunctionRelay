    ...
    [FromBody] Model_Device_Screen_Update_Request updated)
        ...
        public async Task<IActionResult> GetDevicePreferences([FromQuery] string ip)
        {
            ...
            try
            {
                ...
                // Transform ESP32's nested JSON structure to flat structure expected by frontend
                var esp32Data = JsonConvert.DeserializeObject<dynamic>(content);

                var flatPreferences = new Model_Device_Preferences
                {
                    ConnMode = esp32Data?.connectionMode ?? "",
                    WifiSSID = esp32Data?.wifi?.ssid ?? "",
                    WifiPassword = "", // Never returned by ESP32 for security
                    MqttBroker = esp32Data?.mqtt?.broker ?? "",
                    MqttUsername = esp32Data?.mqtt?.username ?? "",
                    MqttPassword = "", // Never returned by ESP32 for security
                    Rotation = esp32Data?.displayRotation ?? 0,
                    SwapBlueGreen = esp32Data?.swapBlueGreen ?? false,
                    Restart = false, // Not a stored preference
                    ExternalNeoPixelsData1 = esp32Data?.externalNeoPixelsData1 ?? "0",
                    ExternalNeoPixelsData2 = esp32Data?.externalNeoPixelsData2 ?? "0"
                };

                return Ok(flatPreferences);
            }
            catch (Exception ex)
            {
