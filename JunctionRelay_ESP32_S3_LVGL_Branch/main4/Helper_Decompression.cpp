...
size_t Helper_Decompression::calculateOptimalBufferSize(size_t compressedSize) {
    if (!psramFound()) {
        // No PSRAM - use heap with conservative estimates
        Serial.println("[DECOMPRESS] No PSRAM - using heap for decompression");
        size_t freeHeap = ESP.getFreeHeap();
        size_t estimatedSize = compressedSize * 10;  // Conservative 10x estimate for compression ratio
        size_t maxHeapBuffer = (freeHeap * 8) / 10;  // Use up to 80% of free heap
        size_t bufferSize = min(estimatedSize, maxHeapBuffer);

        Serial.printf("[DECOMPRESS] Heap-based buffer: %u bytes (free heap: %u)\n",
                      bufferSize, freeHeap);
        return bufferSize;
    }
