...
const char* STREAM_URL = "http://10.168.1.92:60020/stream";
...
#define FRAME_QUEUE_SIZE 10  // Hold up to 10 frames
#define BUFFER_POOL_SIZE 15  // Pool of buffers (larger than queue for safety)
...
volatile int latestReadyIndex = -1;  // Latest complete frame (Core 1 reads this) - SIMPLE AND FAST!
    ...
    for (int i = 0; i < bytesRead; i++) {
      ...
      // Accumulate JPEG data directly in pool buffer
        ...
        if (lastByte == 0xFF && c == 0xD9) {
          ...
          // Release old buffer in this slot if it exists
          if (frameQueue[writeSlot].buffer != nullptr) {
            releaseBuffer(frameQueue[writeSlot].buffer);
          }
          ...
          // Update latest ready index (Core 1 reads this) - SIMPLE!
          int oldLatest = latestReadyIndex;
          latestReadyIndex = writeSlot;

          xSemaphoreGive(queueMutex);

          // Track skipped frames
          if (oldLatest != -1 && oldLatest != writeSlot) {
            framesSkipped++;
          }
...
void loop() {
  ...
  // CORE 1: Grab latest frame from queue and decode - SIMPLE AND FAST!
  xSemaphoreTake(queueMutex, portMAX_DELAY);
  int readSlot = latestReadyIndex;
  xSemaphoreGive(queueMutex);

  if (readSlot != -1) {
    size_t frameSize = frameQueue[readSlot].size;
    uint8_t* frameData = frameQueue[readSlot].buffer;

    // Decode the latest frame
    unsigned long decStart = millis();
    if (jpeg.openRAM(frameData, frameSize, jpegDrawCallback)) {
